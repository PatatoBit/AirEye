...
struct FilterLens: View {
  ...
  var body: some View {
    GeometryReader { geometry in
      ...
      // 4. Interaction: Drag Gesture with minimum distance threshold
      .highPriorityGesture(
        DragGesture(minimumDistance: 10)
          .onChanged { value in
            // Only activate if the drag started near the lens
            if !self.isDragging {
              let distance = sqrt(
                pow(value.startLocation.x - lensPosition.x, 2)
                  + pow(value.startLocation.y - lensPosition.y, 2)
              )
              // Only start dragging if touch started within 80 points of lens center
